 ...
 * @author Dan Boneh
 */ "use strict";
/*jslint indent: 2, bitwise: false, nomen: false, plusplus: false, white: false, regexp: false */ /*global document, window, escape, unescape, module, require, Uint32Array */ /**
...
sjcl.cipher.aes.prototype = {
  ...
  */ /**
   ...
   */ _crypt: function(input, dir) {
        ...
        a = input[0] ^ key[0], b = input[dir ? 3 : 1] ^ key[1], c = input[2] ^ key[2], d = input[dir ? 1 : 3] ^ key[3], a2, b2, c2, nInnerRounds = key.length / 4 - 2, i, kIndex = 4, out = [
            0,
            0,
            0,
            0
 ...
 */ sjcl.bitArray = {
   ...
   */ extract: function(a, bstart, blength) {
        ...
        if ((bstart + blength - 1 ^ bstart) & -32) // it crosses a boundary
        x = a[bstart / 32 | 0] << 32 - sh ^ a[bstart / 32 + 1 | 0] >>> sh;
        else // within a single word
        x = a[bstart / 32 | 0] >>> sh;
   ...
   */ clamp: function(a, len) {
        ...
        var l = a.length;
        len = len & 31;
        if (l > 0 && len) a[l - 1] = sjcl.bitArray.partial(len, a[l - 1] & 0x80000000 >> len - 1, 1);
        return a;
    },
    /**
 ...
 */ sjcl.codec.utf8String = {
    /** Convert from a bitArray to a UTF-8 string. */ fromBits: function(arr) {
    ...
    /** Convert from a UTF-8 string to a bitArray. */ toBits: function(str) {
 ...
 * @namespace
 */ sjcl.codec.hex = {
    /** Convert from a bitArray to a hex string. */ fromBits: function(arr) {
        var out = "", i;
        for(i = 0; i < arr.length; i++)out += ((arr[i] | 0) + 0xF00000000000).toString(16).substr(4);
        return out.substr(0, sjcl.bitArray.bitLength(arr) / 4); //.replace(/(.{8})/g, "$1 ");
    },
    /** Convert from a hex string to a bitArray. */ toBits: function(str) {
        var i, out = [], len;
 ...
 */ sjcl.codec.base32 = {
   ...
   */ _chars: "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567",
    ...
    /* bits in an array */ BITS: 32,
    /* base to encode at (2^x) */ BASE: 5,
    /* bits - base */ REMAINING: 27,
    /** Convert from a bitArray to a base32 string. */ fromBits: function(arr, _noEquals, _hex) {
    ...
    /** Convert from a base32 string to a bitArray */ toBits: function(str, _hex) {
 ...
 */ sjcl.codec.base64 = {
   ...
   */ _chars: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
    /** Convert from a bitArray to a base64 string. */ fromBits: function(arr, _noEquals, _url) {
    ...
    /** Convert from a base64 string to a bitArray */ toBits: function(str, _url) {
        str = str.replace(/\s|=/g, '');
...
sjcl.hash.sha256.prototype = {
   ...
   */ finalize: function() {
        ...
        b = sjcl.bitArray.concat(b, [
            sjcl.bitArray.partial(1, 1)
        ]);
  ...
  */ /**
   ...
   */ _precompute: function() {
        ...
        for(; i < 64; prime++){
            ...
            if (isPrime) {
                if (i < 8) this._init[i] = frac(Math.pow(prime, 0.5));
 ...
 */ sjcl.mode.ccm = {
   ...
   */ name: "ccm",
    ...
    _callProgressListener: function(val) {
        ...
        for(i = 0; i < p.length; i += 1)p[i](val);
    },
    /** Encrypt in CCM mode.
   * @static
   ...
   */ decrypt: function(prf, ciphertext, iv, adata, tlen) {
    ...
    _macAdditionalData: function(prf, adata, iv, tlen, ol, L) {
        ...
        // mac the flags
        mac = [
            w.partial(8, (adata.length ? 64 : 0) | tlen - 2 << 2 | L - 1)
        ];
        ...
        if (adata.length) {
            ...
            if (tmp <= 0xFEFF) macData = [
                w.partial(16, tmp)
            ];
            else if (tmp <= 0xFFFFFFFF) macData = w.concat([
                w.partial(16, 0xFFFE)
            ], [
                tmp
            ]);
   ...
   */ _computeTag: function(prf, plaintext, iv, adata, tlen, L) {
        ...
        if (adata.length > 0xFFFFFFFF || plaintext.length > 0xFFFFFFFF) // I don't want to deal with extracting high words from doubles.
 ...
 */ sjcl.mode.ocb2 = {
   ...
   */ encrypt: function(prp, plaintext, iv, adata, tlen, premac) {
        if (sjcl.bitArray.bitLength(iv) !== 128) throw new sjcl.exception.invalid("ocb iv must be 128 bits");
        var i, times2 = sjcl.mode.ocb2._times2, w = sjcl.bitArray, xor = w._xor4, checksum = [
            0,
            0,
            0,
            0
        ], delta = times2(prp.encrypt(iv)), bi, bl, output = [], pad;
        adata = adata || [];
        tlen = tlen || 64;
        for(i = 0; i + 4 < plaintext.length; i += 4){
            /* Encrypt a non-final block */ bi = plaintext.slice(i, i + 4);
            checksum = xor(checksum, bi);
            output = output.concat(xor(delta, prp.encrypt(xor(delta, bi))));
            delta = times2(delta);
        }
        /* Chop out the final block */ bi = plaintext.slice(i);
        bl = w.bitLength(bi);
        pad = prp.encrypt(xor(delta, [
            0,
            0,
            0,
            bl
        ]));
        bi = w.clamp(xor(bi.concat([
            0,
            0,
            0
        ]), pad), bl);
        /* Checksum the final block, and finalize the checksum */ checksum = xor(checksum, xor(bi.concat([
            0,
            0,
            0
        ]), pad));
        checksum = prp.encrypt(xor(checksum, xor(delta, times2(delta))));
        /* MAC the header */ if (adata.length) checksum = xor(checksum, premac ? adata : sjcl.mode.ocb2.pmac(prp, adata));
   ...
   */ decrypt: function(prp, ciphertext, iv, adata, tlen, premac) {
        ...
        var i, times2 = sjcl.mode.ocb2._times2, w = sjcl.bitArray, xor = w._xor4, checksum = [
            ...
            0,
            0
        ], delta = times2(prp.encrypt(iv)), bi, bl, len = sjcl.bitArray.bitLength(ciphertext) - tlen, output = [], pad;
        adata = adata || [];
        for(i = 0; i + 4 < len / 32; i += 4){
            /* Decrypt a non-final block */ bi = xor(delta, prp.decrypt(xor(delta, ciphertext.slice(i, i + 4))));
            checksum = xor(checksum, bi);
            output = output.concat(bi);
            delta = times2(delta);
        }
        /* Chop out and decrypt the final block */ bl = len - i * 32;
        pad = prp.encrypt(xor(delta, [
            0,
            0,
            0,
        ...
        bi = xor(pad, w.clamp(ciphertext.slice(i), bl).concat([
            0,
            0,
            0
        ]));
        /* Checksum the final block, and finalize the checksum */ checksum = xor(checksum, bi);
        checksum = prp.encrypt(xor(checksum, xor(delta, times2(delta))));
        /* MAC the header */ if (adata.length) checksum = xor(checksum, premac ? adata : sjcl.mode.ocb2.pmac(prp, adata));
        if (!w.equal(w.clamp(checksum, tlen), w.bitSlice(ciphertext, len))) throw new sjcl.exception.corrupt("ocb: tag doesn't match");
 ...
 */ sjcl.mode.gcm = {
   ...
   */ _ctrMode: function(encrypt, prf, data, adata, iv, tlen) {
        ...
        last = [
            Math.floor(abl / 0x100000000),
            abl & 0xffffffff,
            Math.floor(bl / 0x100000000),
            bl & 0xffffffff
        ];
 ...
 */ sjcl.misc.hkdf = function(ikm, keyBitLength, salt, info, Hash) {
    ...
    if (typeof info === "string") info = sjcl.codec.utf8String.toBits(info);
    if (typeof salt === "string") salt = sjcl.codec.utf8String.toBits(salt);
    else if (!salt) salt = [];
    ...
    if (loops > 255) throw new sjcl.exception.invalid("key bit length is too large for hkdf");
 ...
 */ sjcl.prng = function(defaultParanoia) {
    ...
    /* event listener stuff */ this._collectorsStarted = false;
    this._callbacks = {
        progress: {},
        seeded: {}
    };
    this._callbackI = 0;
    /* constants */ this._NOT_READY = 0;
    this._READY = 1;
...
sjcl.prng.prototype = {
   ...
   */ addEntropy: function(data, estimatedEntropy, source) {
            ...
            case "object":
                ...
                if (!err) {
                    if (estimatedEntropy === undefined) {
                        /* horrible entropy estimator */ estimatedEntropy = 0;
                ...
                break;
            case "string":
                if (estimatedEntropy === undefined) /* English text has just over 1 bit per character of entropy.
        ...
        */ estimatedEntropy = data.length;
        ...
        /* record the new strength */ this._poolEntropy[robin] += estimatedEntropy;
        this._poolStrength += estimatedEntropy;
        /* fire off events */ if (oldReady === this._NOT_READY) {
            if (this.isReady() !== this._NOT_READY) this._fireEvent("seeded", Math.max(this._strength, this._poolStrength));
            this._fireEvent("progress", this.getProgress());
        }
    },
    /** Is the generator ready? */ isReady: function(paranoia) {
    ...
    /** Get the generator's progress toward readiness, as a fraction */ getProgress: function(paranoia) {
        var entropyRequired = this._PARANOIA_LEVELS[paranoia ? paranoia : this._defaultParanoia];
        if (this._strength >= entropyRequired) return 1.0;
        else return this._poolStrength > entropyRequired ? 1.0 : this._poolStrength / entropyRequired;
    },
    /** start the built-in entropy collectors */ startCollectors: function() {
    ...
    /** stop the built-in entropy collectors */ stopCollectors: function() {
        if (!this._collectorsStarted) return;
  ...
  },*/ /** add an event listener for progress or seeded-ness. */ addEventListener: function(name, callback) {
        this._callbacks[name][this._callbackI++] = callback;
    },
    /** remove an event listener for progress or seeded-ness */ removeEventListener: function(name, cb) {
   ...
   */ _reseedFromPools: function(full) {
        ...
        for(i = 0; i < 16; i++)/* On some browsers, this is cryptographically random.  So we might
       ...
       */ reseedData.push(Math.random() * 0x100000000 | 0);
        ...
        /* if we used the last pool, push a new one onto the stack */ if (this._reseedCount >= 1 << this._pools.length) {
            this._pools.push(new sjcl.hash.sha256());
            this._poolEntropy.push(0);
        }
        /* how strong was this reseed? */ this._poolStrength -= strength;
    ...
    _addCurrentTimeToEntropy: function(estimatedEntropy) {
        if (typeof window !== 'undefined' && window.performance && typeof window.performance.now === "function") //how much entropy do we want to add here?
        this.addEntropy(window.performance.now(), estimatedEntropy, "loadtime");
        else this.addEntropy(new Date().valueOf(), estimatedEntropy, "loadtime");
    },
    ...
    _fireEvent: function(name, arg) {
        ...
        /* TODO: there is a race condition between removing collectors and firing them */ /* I'm not sure if this is necessary; in C++, iterating over a
     * collection and modifying it at the same time is a no-no.
 ...
 */ /**
  ...
  * @namespace
  */ sjcl.json = {
    /** Default values for encryption */ defaults: {
        v: 1,
        iter: 10000,
        ks: 128,
        ts: 64,
   ...
   */ _encrypt: function(password, plaintext, params, rp) {
        ...
        /* return the json data */ j._add(rp, p);
        rp.key = password;
        /* do the encryption */ if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && plaintext instanceof ArrayBuffer) p.ct = sjcl.arrayBuffer.ccm.encrypt(prp, plaintext, p.iv, adata, p.ts);
   ...
   */ _decrypt: function(password, ciphertext, params, rp) {
        ...
        prp = new sjcl.cipher[p.cipher](password);
        /* do the decryption */ if (p.mode === "ccm" && sjcl.arrayBuffer && sjcl.arrayBuffer.ccm && p.ct instanceof ArrayBuffer) ct = sjcl.arrayBuffer.ccm.decrypt(prp, p.ct, p.iv, p.tag, adata, p.ts);
   ...
   */ _add: function(target, src, requireSame) {
        ...
        for(i in src)if (src.hasOwnProperty(i)) {
            if (requireSame && target[i] !== undefined && target[i] !== src[i]) throw new sjcl.exception.invalid("required parameter overridden");
            target[i] = src[i];
        }
        return target;
    },
   ...
   */ _filter: function(src, filter) {
        var out = {}, i;
        for(i = 0; i < filter.length; i++)if (src[filter[i]] !== undefined) out[filter[i]] = src[filter[i]];
        return out;
    }
};
 ...
 */ sjcl.misc.cachedPbkdf2 = function(password, obj) {
    ...
    obj = obj || {};
    iter = obj.iter || 1000;
    /* open the cache for this password and iteration count */ cp = cache[password] = cache[password] || {};
    c = cp[iter] = cp[iter] || {
...
sjcl.bn.prototype = {
   ...
   */ toString: function() {
    ...
    /** this += that.  Does not normalize. */ addM: function(that) {
    ...
    /** this *= 2.  Requires normalized; ends up normalized. */ doubleM: function() {
    ...
    /** this /= 2, rounded down.  Requires normalized; ends up normalized. */ halveM: function() {
    ...
    /** this -= that.  Does not normalize. */ subM: function(that) {
        if (typeof that !== "object") that = new this._class(that);
        var i, l = this.limbs, ll = that.limbs;
        for(i = l.length; i < ll.length; i++)l[i] = 0;
        for(i = 0; i < ll.length; i++)l[i] -= ll[i];
    ...
    /** return inverse mod prime p.  p must be odd. Binary extended Euclidean algorithm mod p. */ inverseMod: function(p) {
    ...
    /** this + that.  Does not normalize. */ add: function(that) {
        return this.copy().addM(that);
    },
    /** this - that.  Does not normalize. */ sub: function(that) {
        return this.copy().subM(that);
    },
    /** this * that.  Normalizes and reduces. */ mul: function(that) {
        if (typeof that === "number") that = new this._class(that);
        else that.normalize();
        this.normalize();
    ...
    /** this ^ 2.  Normalizes and reduces. */ square: function() {
        return this.mul(this);
    },
    /** this ^ n.  Uses square-and-multiply.  Normalizes and reduces. */ power: function(l) {
    ...
    /** this * that mod N */ mulmod: function(that, N) {
        return this.mod(N).mul(that.mod(N)).mod(N);
    },
    /** this ^ x mod N */ powermod: function(x, N) {
        x = new sjcl.bn(x);
        N = new sjcl.bn(N);
    ...
    /** this ^ x mod N with Montomery reduction */ montpowermod: function(x, N) {
    ...
    /** Reduce mod a modulus.  Stubbed for subclassing. */ reduce: function() {
        return this;
    },
    /** Reduce and normalize. */ fullReduce: function() {
        return this.normalize();
    },
    /** Propagate carries. */ normalize: function() {
    ...
    /** Constant-time normalize. Does not allocate additional space. */ cnormalize: function() {
    ...
    /** Serialize to a bit array */ toBits: function(len) {
        ...
        var i = Math.floor((len - 1) / 24), w = sjcl.bitArray, e = (len + 7 & -8) % this.radix || this.radix, out = [
            w.partial(e, this.getLimb(i))
        ];
        for(i--; i >= 0; i--){
            out = w.concat(out, [
    ...
    /** Return the length in bits, rounded up to the nearest byte. */ bitLength: function() {
        this.fullReduce();
 ...
 */ sjcl.ecc.point = function(curve, x, y) {
    if (x === undefined) this.isIdentity = true;
    else {
        if (x instanceof sjcl.bn) x = new curve.field(x);
        if (y instanceof sjcl.bn) y = new curve.field(y);
        this.x = x;
        this.y = y;
    ...
    multiples: function() {
        ...
        if (this._multiples === undefined) {
            ...
            this._multiples = [
                ...
                this
            ].concat(sjcl.ecc.pointJac.toAffineMultiple(m));
        }
 ...
 */ sjcl.ecc.pointJac.toAffineMultiple = function(points) {
    ...
    for(; i < points.length; i++){
        ...
        if (curve != p.curve) {
            ...
            if (curve) {
                ...
                for(i = 0; i < points.length; i++)ret[i] = points[i].toAffine();
                return ret;
            } else curve = p.curve;
        }
    ...
    for(i--; i >= 0; i--){
        p = points[i];
        if (p.isIdentity || p.z.equals(0)) ret[i] = new sjcl.ecc.point(p.curve);
        else {
            // if not last
            if (j >= 0) {
...
sjcl.ecc.pointJac.prototype = {
   ...
   */ add: function(T) {
        ...
        if (S.curve !== T.curve) throw new sjcl.exception.invalid("sjcl.ecc.add(): Points must be on the same curve to add them!");
        if (S.isIdentity) return T.toJac();
        else if (T.isIdentity) return S;
        sz2 = S.z.square();
        c = T.x.mul(sz2).subM(S.x);
        if (c.equals(0)) {
            if (S.y.equals(T.y.mul(sz2.mul(S.z)))) // same point
            return S.doubl();
            else // inverses
   ...
   */ mult: function(k, affine) {
        ...
        else if (k.limbs !== undefined) k = k.normalize().limbs;
        var i, j, out = new sjcl.ecc.point(this.curve).toJac(), multiples = affine.multiples();
        for(i = k.length - 1; i >= 0; i--)for(j = sjcl.bn.prototype.radix - 4; j >= 0; j -= 4)out = out.doubl().doubl().doubl().doubl().add(multiples[k[i] >> j & 0xF]);
        return out;
    },
    /**
   * Multiply this point by k, added to affine2*k2, and return the answer in Jacobian coordinates.
   ...
   */ mult2: function(k1, affine, k2, affine2) {
        if (typeof k1 === "number") k1 = [
            k1
        ];
        else if (k1.limbs !== undefined) k1 = k1.normalize().limbs;
        if (typeof k2 === "number") k2 = [
            k2
        ];
        else if (k2.limbs !== undefined) k2 = k2.normalize().limbs;
        var i, j, out = new sjcl.ecc.point(this.curve).toJac(), m1 = affine.multiples(), m2 = affine2.multiples(), l1, l2;
        for(i = Math.max(k1.length, k2.length) - 1; i >= 0; i--){
            l1 = k1[i] | 0;
            l2 = k2[i] | 0;
            for(j = sjcl.bn.prototype.radix - 4; j >= 0; j -= 4)out = out.doubl().doubl().doubl().doubl().add(m1[l1 >> j & 0xF]).add(m2[l2 >> j & 0xF]);
        }
        return out;
    },
    negate: function() {
...
sjcl.ecc.curve.prototype.fromBits = function(bits) {
    ...
    if (!p.isValid()) throw new sjcl.exception.corrupt("not on the curve!");
    return p;
};
sjcl.ecc.curves = {
...
sjcl.ecc.curveName = function(curve) {
    var curcurve;
    for(curcurve in sjcl.ecc.curves)if (sjcl.ecc.curves.hasOwnProperty(curcurve)) {
        if (sjcl.ecc.curves[curcurve] === curve) return curcurve;
    }
...
*/ sjcl.ecc.basicKey = {
  ...
  */ publicKey: function(curve, point) {
        ...
        if (point instanceof Array) this._point = curve.fromBits(point);
        else this._point = point;
...
/** @private */ sjcl.ecc.basicKey.generateKeys = function(cn) {
    return function generateKeys(curve, paranoia, sec) {
        curve = curve || 256;
        if (typeof curve === "number") {
            curve = sjcl.ecc.curves['c' + curve];
            if (curve === undefined) throw new sjcl.exception.invalid("no such curve");
        }
...
/** specific functions for ecdsa publicKey. */ sjcl.ecc.ecdsa.publicKey.prototype = {
    /** Diffie-Hellmann function
  * @param {bitArray} hash hash to verify.
  * @param {bitArray} rs signature bitArray.
  * @param {boolean}  fakeLegacyVersion use old legacy version
  */ verify: function(hash, rs, fakeLegacyVersion) {
        if (sjcl.bitArray.bitLength(hash) > this._curveBitLength) hash = sjcl.bitArray.clamp(hash, this._curveBitLength);
        ...
        if (r.equals(0) || ss.equals(0) || r.greaterEquals(R) || ss.greaterEquals(R) || !r2.equals(r)) {
            if (fakeLegacyVersion === undefined) return this.verify(hash, rs, true);
            else throw new sjcl.exception.corrupt("signature didn't check out");
        }
        return true;
    },
    getType: function() {
...
/** specific functions for ecdsa secretKey. */ sjcl.ecc.ecdsa.secretKey.prototype = {
    /** Diffie-Hellmann function
...
//# sourceMappingURL=smarthub.2fd0bd96.js.map
